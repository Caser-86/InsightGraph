  ...
  <script>
    ...
    const I18N = {
      zh: {
        ...
        searchProviderExpression: '搜索提供方表达式',
        resolvedSearchProviders: '实际搜索提供方',
        serpapiEnabled: 'SerpAPI 已启用',
        searchLimit: '搜索数量',
        ...
        researchJobsBackend: '任务存储后端',
        researchJobsJsonPath: '任务 JSON 路径',
        researchJobsSqlitePath: '任务 SQLite 路径',
        eventRetentionLimit: '事件保留上限',
        yes: '是',
        no: '否',
        evalOps: '评测运维',
      ...
      en: {
        ...
        searchProviderExpression: 'Search provider expression',
        resolvedSearchProviders: 'Resolved search providers',
        serpapiEnabled: 'SerpAPI enabled',
        ...
        researchJobsBackend: 'Jobs backend',
        researchJobsJsonPath: 'Jobs JSON path',
        researchJobsSqlitePath: 'Jobs SQLite path',
        eventRetentionLimit: 'Event retention limit',
        yes: 'yes',
        no: 'no',
        evalOps: 'Eval Ops',
    ...
    function renderRuntimeDiagnostics(result) {
      ...
      const items = [
        ...
        [t('searchProviderExpression'), diagnostics.search_provider_expression || t('unknown')],
        [t('resolvedSearchProviders'), (diagnostics.resolved_search_providers || []).join(', ') || t('unknown')],
        [t('serpapiEnabled'), diagnostics.serpapi_enabled ? t('yes') : t('no')],
        ...
        [t('researchJobsBackend'), diagnostics.research_jobs_backend || t('unknown')],
        [t('researchJobsJsonPath'), diagnostics.research_jobs_json_path || t('unknown')],
        [t('researchJobsSqlitePath'), diagnostics.research_jobs_sqlite_path || t('unknown')],
        [t('eventRetentionLimit'), diagnostics.event_retention_limit ?? t('unknown')],
      ];
      return `<div class="overview-grid">${items.map(([label, value]) => `<div class="info-card"><span>${escapeHtml(label)}</span><strong>${escapeHtml(value)}</strong></div>`).join('')}</div>`;
    }
