...
Report Quality v3、Live Benchmark Case Profiles、Production RAG Hardening、Memory Quality Loop、Dashboard Productization、API/Operations Hardening 已完成，当前状态为 A-F complete。详细状态见 `docs/roadmap.md`。

剩余工作都需要显式决策：优先考虑 `/tasks` API compatibility aliases；MCP runtime invocation、release/deploy automation dry-run only、真实 sandboxed Python/code execution 继续保持 deferred，直到有明确使用场景和安全评审。
