...
- **任务追踪**：目标态通过 `task_id` 与 `thread_id` 关联用户请求、Graph 状态和最终报告；当前 API jobs 支持状态数量 summary、按状态与数量上限列出任务摘要、按 `job_id` 查询详情、取消尚未执行的 `queued` jobs、从 `failed` / `cancelled` source 手动 retry，并返回 `created_at` / `started_at` / `finished_at` UTC 时间 metadata；`queued` jobs 还返回动态计算的 1-based `queue_position`；job store 只保留最近 100 个 `succeeded` / `failed` / `cancelled` jobs，`queued` / `running` jobs 不会被裁剪，但 `queued + running` 达到 active cap 后会拒绝新 job；API jobs 现在有显式初始化边界与状态 rollback snapshot helper，默认使用进程内存，可通过 `INSIGHT_GRAPH_RESEARCH_JOBS_PATH` opt-in JSON metadata 持久化，或通过 SQLite backend 获得内部 worker lease、claim、heartbeat 与 expired running requeue；PostgreSQL checkpoint 与自动恢复执行仍未实现
