...
Manual live benchmark runs use `scripts/benchmark_live_research.py --allow-live --output reports/live-benchmark.json`, `scripts/benchmark_live_research.py --allow-live --output reports/live-benchmark.json --case-file docs/benchmarks/live-research-cases.json`, or `INSIGHT_GRAPH_ALLOW_LIVE_BENCHMARK=1`. This path always uses `live-research`, can access real network/LLM providers, and may incur network/LLM cost. Without explicit opt-in the script exits without writing an artifact. Do not commit generated live benchmark reports. The artifact is a metrics summary with fields such as `url_validation_rate`, `citation_precision_proxy`, `source_diversity_by_type`, `source_diversity_by_domain`, `section_coverage`, and `total_tokens`.

Use `--preset live-research` for a reference-style networked research run. It sets `INSIGHT_GRAPH_USE_WEB_SEARCH=1`, `INSIGHT_GRAPH_SEARCH_PROVIDER=duckduckgo`, `INSIGHT_GRAPH_SEARCH_LIMIT=20`, `INSIGHT_GRAPH_USE_GITHUB_SEARCH=1`, `INSIGHT_GRAPH_GITHUB_PROVIDER=live`, `INSIGHT_GRAPH_USE_SEC_FILINGS=1`, `INSIGHT_GRAPH_USE_SEC_FINANCIALS=1`, `INSIGHT_GRAPH_MULTI_SOURCE_COLLECTION=1`, `INSIGHT_GRAPH_MAX_COLLECTION_ROUNDS=5`, `INSIGHT_GRAPH_MAX_TOOL_CALLS=200`, `INSIGHT_GRAPH_MAX_FETCHES=80`, `INSIGHT_GRAPH_MAX_EVIDENCE_PER_RUN=120`, `INSIGHT_GRAPH_REPORTER_VALIDATE_URLS=1`, `INSIGHT_GRAPH_RELEVANCE_FILTER=1`, `INSIGHT_GRAPH_RELEVANCE_JUDGE=openai_compatible`, `INSIGHT_GRAPH_ANALYST_PROVIDER=llm`, and `INSIGHT_GRAPH_REPORTER_PROVIDER=llm`. Fetched responses are size-bounded and oversized `Content-Length` responses are rejected before body reads; long HTML pages are split into bounded evidence chunks with `chunk_index` and `section_heading` metadata; fetched PDF responses emit docs evidence with `chunk_index` and `document_page` metadata. JavaScript-rendered fetch is optional via `INSIGHT_GRAPH_FETCH_RENDERED=1` and requires Playwright to be installed separately. Source URLs are canonicalized for dedupe, source types are classified as `official_site`, `docs`, `github`, `news`, `blog`, `sec`, `paper`, or `unknown`, and fetch failures carry stable error kinds such as `network`, `http_status`, `content_type`, `too_large`, and `blocked_url`.
...
| `INSIGHT_GRAPH_SEARCH_PROVIDER` | `mock`、`duckduckgo`、`google` 或 `serpapi` | `mock` |
...
| `INSIGHT_GRAPH_SERPAPI_KEY` / `INSIGHT_GRAPH_SERPAPI_API_KEY` / `SERPAPI_API_KEY` | SerpAPI API key，三种变量名任选其一 | - |
| `INSIGHT_GRAPH_SERPAPI_ENGINE` | SerpAPI engine，例如 `google` | `google` |
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `INSIGHT_GRAPH_MAX_TOOL_CALLS` | 标准版单次研究最多执行的 tool call 数 | `200` |
| `INSIGHT_GRAPH_MAX_STEPS` | 预留的 workflow step budget，供后续 agent loop/checkpoint 使用 | `10` |
| `INSIGHT_GRAPH_MAX_FETCHES` | 标准版 pre-fetch candidate 抓取上限；候选会先按 canonical URL 去重 | `80` |
| `INSIGHT_GRAPH_MAX_EVIDENCE_PER_RUN` | 标准版单次研究最终保留的 evidence 数上限 | `120` |
| `INSIGHT_GRAPH_MAX_TOKENS` | 单次研究已记录 LLM total token 上限；耗尽后 Analyst/Reporter/Report Review 回退 deterministic，OpenAI relevance judge 保守拒绝新 LLM 判断 | `500000` |
| `INSIGHT_GRAPH_REPORT_INTENSITY` | 报告强度：`concise`、`standard`、`deep`、`deep-plus`；默认 `standard` | `standard` |
