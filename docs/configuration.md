...
Manual live benchmark runs use `scripts/benchmark_live_research.py --allow-live --output reports/live-benchmark.json` or `INSIGHT_GRAPH_ALLOW_LIVE_BENCHMARK=1`. This path always uses `live-research`, can access real network/LLM providers, and may incur network/LLM cost. Without explicit opt-in the script exits without writing an artifact.
