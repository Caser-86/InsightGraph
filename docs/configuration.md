...
`PgVectorResearchMemoryStore` stores `memory_id`, text, embedding vector, and JSON metadata in `insight_graph_memories`. It provides persistence/search plus deletion by memory ID or metadata key/value. `build_memory_record` can generate deterministic offline embeddings for process-local tests and reproducible memory records. Real embedding providers and report-quality eval proof remain future work.
